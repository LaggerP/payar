              ...
              <div className="shadow overflow-hidden sm:rounded-md">
                <div className="px-4 py-5 bg-white sm:p-6">
                  <div className="flex flex-col space-y-5">
                    <div className="">
                      <label htmlFor="productDescription" className="block text-sm font-medium text-gray-700">Descripción del producto a vender</label>
                      <input type="text" name="productDescription" id="productDescription" autoComplete="none" className="mt-1 p-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border border-gray-300 rounded-md" placeholder="Ingrese una descripción del producto que va a cobrar" required onChange={handleChange}/>
                    </div>
                    <div className="">
                      <label htmlFor="cryptoCoin" className="block text-sm font-medium text-gray-700">Selecciones cripto a usar</label>
                    ...
                    <div className="">
                      <label htmlFor="cryptoAddress" className="block text-sm font-medium text-gray-700">Selecciones direccion de billetera (Wallet Address)</label>
                      <select id="cryptoAddress" name="cryptoAddress" className="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required onChange={handleChange}>
                        <option selected="selected">Seleccione dirección asociada a la criptomoneda seleccionada</option>
                        {
                          data.map((address, idx) =>
                            <option key={idx}>{address.cryptoAddress}</option>
                          )
                        }
                      </select>
                    </div>
                    <div className="col-span-6">
                      <label htmlFor="productPrice" className="block text-sm font-medium text-gray-700">Monto en pesos (ARS)</label>
                      <input type="number" min="1" step="any" name="productPrice" id="productPrice" autoComplete="none" className="mt-1 p-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border border-gray-300 rounded-md" placeholder="Ingrese el monto en pesos" required onChange={handleChange}/>
                    </div>
