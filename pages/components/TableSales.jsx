  ...
  const confirmSale = async () => {
    try {
      const res = await fetch('/api/cobros', {
        ...
        body: JSON.stringify(props.cobro._id)
      })
      if (!res.ok) {
               ...
               <div className="flex items-center">
                  ...
                  <div className="ml-4">
                     <div className="text-md font-medium text-gray-900">
                        {props.cobro.productDescription}
                     </div>
                     <div className="text-sm text-indigo-800">
                        <a href={props.cobro.qrUrl} target="_blank" rel="noreferrer">Abrir QR</a>
                     </div>
                  </div>
               </div>
            </td>
            <td className="px-6 py-4 whitespace-nowrap">
               <div className="text-md text-gray-900 font-medium pb-2">${props.cobro.productPrice}</div>
               <div className="text-xs text-gray-500 inline-flex items-center">{cryptoIcon(props.cobro.cryptoCoin)}
                  <span className="pl-1">
                     {props.cobro.cryptoAddress}
                  </span>
               </div>
            </td>
            <td className="px-6 py-4 whitespace-nowrap">
               {
                  props.cobro.transactionStatus
            ...
            <td className="px-8 py-4 whitespace-nowrap">
               {
                  props.cobro.transactionStatus
                    ...
                    : <>
                        <button className="mr-2 focus:outline-none" title="Confirmar recepción del pago" onClick={() => confirmSale()}>
                        ...
                        <button className="ml-2 focus:outline-none" title="No confirmar recepción del pago" onClick={() => deleteSale()}>
